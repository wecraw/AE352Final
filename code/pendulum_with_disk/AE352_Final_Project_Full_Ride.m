...
title('\theta vs. t, Disk Pendulum')

figure;
plot(t_out,y_out(:,2));
xlabel('Time, t, seconds')
ylabel('Pendulum rotation rate, \omega, radians/second')
title('\omega vs. t, Disk Pendulum')
...
for ridernumber=1:Numpeople
    ...
    plot(t_out,aplot./9.81);
    hold on;
end
xlabel('Time, t, seconds');
ylabel('Acceleration magnitude, a, shown in g');
title('Acceleration Magnitude vs. Time for duration of ride (for one rider)');
